...
import 'package:clubevent/data/repositories/chat/firestore_database.dart';
import 'package:clubevent/data/repositories/chat/realtime_database.dart';
...
class ChatManager {
  ...
  Future<void> deleteMessage(String chatReference, Mensaje message) async {
    await realtime.deleteMessage(
        chatPath: chatReference, messagePath: message.reference!);
  }

  List<Mensaje> extractMessages(DataSnapshot snapshot) {
    final chatsData = realtime.extractDocs(snapshot);
    return _extractMessagesInstances(chatsData);
  }

  List<Mensaje> _extractMessagesInstances(List<Map<String, dynamic>> data) {
    List<Mensaje> chats = [];
    for (var chat in data) {
      chats.add(
        Mensaje.fromJson(chat),
      );
    }
    return chats;
  }
